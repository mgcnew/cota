...
/**
 * Hook ultra-otimizado para mobile
 * - Paginação server-side
 * - Filtro de categoria no servidor
 * - Apenas campos essenciais
 * - Lazy loading avançado
 */
export function useProductsMobileOptimized(
  searchQuery?: string,
  ...
  // Query principal com paginação e filtros server-side
  const pagination = useServerPagination<ProductMobile>({
    queryKey: ['products-mobile-optimized', searchQuery, categoryFilter],
  ...
  const createProduct = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['products-mobile-optimized'] });
      queryClient.invalidateQueries({ queryKey: ['product-categories-mobile'] });
      toast({
