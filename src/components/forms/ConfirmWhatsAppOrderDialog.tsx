...
interface ConfirmWhatsAppOrderDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  pedido: OrderData | null;
  onConfirm: () => void; // Dispara quando clica p/ alterar o status
}

export function ConfirmWhatsAppOrderDialog({
  open,
  onOpenChange,
  pedido,
  onConfirm,
}: ConfirmWhatsAppOrderDialogProps) {
  ...
  const loadMessage = async () => {
    ...
    try {
      ...
      const res = await generateOrderMessage(pedido!.id);
