...
function LogoComponent() {
  return (
    <div className="w-10 h-10 bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-600 rounded-lg flex items-center justify-center shadow-lg relative overflow-hidden">
...
const colors = [
  { bg: 'from-blue-500 to-blue-600', shadow: 'shadow-blue-500/25', from: '#3b82f6', to: '#2563eb', shadowColor: 'rgba(59, 130, 246, 0.25)', glow: 'shadow-blue-400/50' },
  { bg: 'from-orange-500 to-amber-500', shadow: 'shadow-orange-500/25', from: '#f97316', to: '#f59e0b', shadowColor: 'rgba(249, 115, 22, 0.25)', glow: 'shadow-orange-400/50' },
  { bg: 'from-indigo-500 to-blue-500', shadow: 'shadow-indigo-500/25', from: '#6366f1', to: '#3b82f6', shadowColor: 'rgba(99, 102, 241, 0.25)', glow: 'shadow-indigo-400/50' },
  { bg: 'from-teal-500 to-cyan-500', shadow: 'shadow-teal-500/25', from: '#14b8a6', to: '#06b6d4', shadowColor: 'rgba(20, 184, 166, 0.25)', glow: 'shadow-teal-400/50' },
  { bg: 'from-pink-500 to-rose-500', shadow: 'shadow-pink-500/25', from: '#ec4899', to: '#f43f5e', shadowColor: 'rgba(236, 72, 153, 0.25)', glow: 'shadow-pink-400/50' },
  { bg: 'from-slate-500 to-gray-500', shadow: 'shadow-slate-500/25', from: '#64748b', to: '#6b7280', shadowColor: 'rgba(100, 116, 139, 0.25)', glow: 'shadow-slate-400/50' },
  { bg: 'from-purple-500 to-violet-500', shadow: 'shadow-purple-500/25', from: '#a855f7', to: '#8b5cf6', shadowColor: 'rgba(168, 85, 247, 0.25)', glow: 'shadow-purple-400/50' },
  { bg: 'from-green-500 to-emerald-500', shadow: 'shadow-green-500/25', from: '#22c55e', to: '#10b981', shadowColor: 'rgba(34, 197, 94, 0.25)', glow: 'shadow-green-400/50' }
];

// Componente do botão "Mais" para mobile
                ...
                return (
                  <NavLink
                    ...
                    className={cn(
                      ...
                      isItemActive
                        ? `bg-gradient-to-br ${itemColor.bg} shadow-lg text-white ring-2 ring-white/20`
                  ...
                  >
                    <div className={cn(
                      ...
                      isItemActive
                        ...
                        : `bg-gradient-to-br ${itemColor.bg} group-hover:scale-110`
                    )}>
...
export function AppSidebar() {
  ...
  // Hook para detectar scroll e aplicar animações no sidebar
  useEffect(() => {
    const handleScroll = () => {
      ...
      <div className="hidden md:flex fixed z-50 w-20 left-4 top-2 bottom-4 transition-all duration-500 ease-out">
        ...
        <div className="w-full flex flex-col bg-white/98 backdrop-blur-3xl rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.12)] border border-gray-200/60 hover:shadow-[0_12px_48px_rgba(0,0,0,0.15)] hover:border-gray-300/70 hover:bg-white transition-all duration-500">
          ...
          <>
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-white/50 via-white/20 to-white/40 rounded-2xl"></div>
            {/* Borda interna sutil */}
            <div className="absolute inset-0 rounded-2xl ring-1 ring-inset ring-white/60"></div>
          </>
          ...
          <div className="relative z-10 flex flex-col h-full">
            ...
            <div className="flex-1 flex flex-col justify-start py-2 px-2 transition-all duration-500">
                      ...
                      <Tooltip key={item.title}>
                        <TooltipTrigger asChild>
                          <NavLink
                            ...
                            className={cn(
                              "flex flex-col items-center justify-center transition-all duration-500 group relative rounded-lg h-14 p-1.5",
                              isItemActive
                                ? `bg-gradient-to-br ${itemColor.bg} shadow-[0_4px_16px_${itemColor.shadowColor}] border border-white/40`
                                : "text-gray-600 hover:text-gray-800 hover:bg-white/80 hover:shadow-[0_2px_8px_rgba(0,0,0,0.06)] hover:border-gray-200/60 bg-white/40 border border-transparent hover:border-gray-200/50"
                            )}
                          >
                            {/* Indicador de ativo */}
                            {isItemActive && (
                              <div className="absolute -left-0.5 top-1/2 -translate-y-1/2 w-0.5 h-6 bg-white rounded-r-full shadow-[0_2px_8px_rgba(255,255,255,0.6)] transition-all duration-500"></div>
                            )}



                            {/* Ícone responsivo - Estilo Canva */}
                            <div className="flex items-center justify-center mb-1">
                              <item.icon className={cn(
                                "transition-all duration-500 flex-shrink-0 h-4.5 w-4.5",
                                isItemActive
                            ...
                            {/* Texto responsivo - Estilo Canva */}
                            <span className={cn(
                              "font-semibold text-center leading-tight transition-all duration-500 text-[8.5px]",
                              "w-full truncate px-0.5 tracking-wide",
                              isItemActive
                                ? "text-white drop-shadow-lg"
                                : "text-gray-600 group-hover:text-gray-800"
                            )}>
              ...
              <NavLink
                ...
                style={{
                  background: isItemActive
                    ? `linear-gradient(135deg, ${itemColor.from}, ${itemColor.to})`
                    : 'transparent',
                  boxShadow: isItemActive
                    ? `0 8px 25px -5px ${itemColor.shadow}, 0 4px 10px -3px ${itemColor.shadow}`
                    : 'none',
