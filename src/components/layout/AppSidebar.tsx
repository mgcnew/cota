                ...
                <div className="space-y-1">
                  ...
                  return <Tooltip key={item.title}>
                        <TooltipTrigger asChild>
                          <NavLink to={item.url} end={item.url === "/"} className={cn("flex flex-col items-center justify-center transition-all duration-500 group relative rounded-lg h-14 p-1.5", isItemActive ? `bg-gradient-to-br ${itemColor.bg} shadow-[0_4px_16px_${itemColor.shadowColor}] border border-white/40` : "text-gray-600 hover:text-gray-800 hover:bg-white/80 hover:shadow-[0_2px_8px_rgba(0,0,0,0.06)] hover:border-gray-200/60 bg-white/40 border border-transparent hover:border-gray-200/50")}>
                            {/* Indicador de ativo */}
                            ...
                            <div className="flex items-center justify-center mb-1">
                              <item.icon className={cn("transition-all duration-500 flex-shrink-0 h-4.5 w-4.5", isItemActive ? "text-white drop-shadow-lg" : "text-gray-600 group-hover:text-gray-800")} />
                            </div>

                            {/* Texto responsivo - Estilo Canva */}
                            <span className={cn("font-semibold text-center leading-tight transition-all duration-500 text-[8.5px]", "w-full truncate px-0.5 tracking-wide", isItemActive ? "text-white drop-shadow-lg" : "text-gray-600 group-hover:text-gray-800")}>
                              {item.title}
                            </span>
                          </NavLink>
                        </TooltipTrigger>
                        <TooltipContent side="right" className="font-medium bg-white text-gray-900 border-gray-200 shadow-xl">
                          {item.title}
                        </TooltipContent>
                      </Tooltip>;
                })}
