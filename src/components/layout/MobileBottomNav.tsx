...
const MORE_NAV_ITEMS: { label: string; icon?: ElementType; iconSrc?: string; href: string }[] = [
  { label: "Fornecedores", iconSrc: fornecedoresIcon, href: "/dashboard/fornecedores" },
  { label: "Anotações",   icon: StickyNote,  href: "/dashboard/anotacoes"   },
  { label: "Relatórios",  icon: BarChart3,   iconSrc: relatoriosIcon, href: "/dashboard/relatorios"  },
  { label: "Embalagens",  icon: PackageOpen, iconSrc: embalagensIcon, href: "/dashboard/embalagens" },
  { label: "Análise",     icon: TrendingUp,  href: "/dashboard/analise-compras" },
  { label: "Etiquetas",  icon: ScanLine,    iconSrc: etiquetasIcon, href: "/dashboard/etiquetas" },
];
    ...
    <>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-[#16181C]/95 backdrop-blur-xl border-t border-border dark:border-white/5 shadow-[0_-4px_24px_-8px_rgba(0,0,0,0.12)]" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
        <div className="flex items-stretch h-16">
          ...
          <PrimaryTab label="Produtos"  icon={Package}         iconSrc={produtosIcon} active={isProdutos}  onClick={() => go("/dashboard/produtos")} />
          <PrimaryTab label="Compras"   icon={ShoppingCart}    iconSrc={comprasIcon}  active={isCompras}   onClick={() => go("/dashboard/compras")} />
          <button
        ...
        <DrawerContent className="md:hidden rounded-t-2xl pb-safe bg-background border-t border-border dark:border-white/5 focus:outline-none">
          ...
          <div className="px-4 pb-6 pt-2 space-y-4">
            ...
            <div className="grid grid-cols-4 gap-y-4 gap-x-2">
              {MORE_NAV_ITEMS.map(({ label, icon: Icon, iconSrc, href }) => {
