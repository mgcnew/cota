...
export function EditShoppingListItemDialog({
  ...
  const modalInnerContent = (
    <>
      {/* Header */}
      <div className={`bg-gradient-to-r from-blue-500 to-indigo-600 ${isMobile ? 'p-5' : 'p-6'} text-white ${isMobile ? 'rounded-t-2xl' : ''}`}>
        <div className="flex items-center gap-4">
          <div className={`${isMobile ? 'w-11 h-11' : 'w-12 h-12'} rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center`}>
            <Edit3 className={isMobile ? 'w-5 h-5' : 'w-6 h-6'} />
          </div>
          <div>
      ...
      {/* Product Info */}
      <div className={`${isMobile ? 'px-5 py-3' : 'px-6 py-4'} border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50`}>
      ...
      {/* Form */}
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className={`${isMobile ? 'p-5' : 'p-6'} space-y-5`}>
          {/* Quantity */}
          <FormField
                ...
                <FormControl>
                  <div className="flex gap-2 items-center">
                    <Input
                      ...
                      className={`flex-1 ${isMobile ? 'text-base h-11' : 'text-lg'} font-semibold`}
                    />
                    <div className={`px-4 ${isMobile ? 'py-3' : 'py-2'} bg-gray-100 dark:bg-gray-800 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-400 min-w-[60px] text-center`}>
                      {item.unit}
                    </div>
                  </div>
                </FormControl>
      ...
      {/* Footer */}
      <div className={`${isMobile ? 'p-4' : 'p-4'} border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex gap-3`}>
        <Button
          type="button"
          variant="outline"
        ...
        <Button
          ...
          className={`flex-1 ${isMobile ? 'h-11' : ''} bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700`}
        >
  ...
  if (isMobile) {
    return (
      <Drawer open={open} onOpenChange={onOpenChange}>
        <DrawerContent className="h-[90vh] rounded-t-2xl p-0 overflow-hidden flex flex-col bg-white dark:bg-gray-900">
          {modalInnerContent}
        </DrawerContent>
      </Drawer>
    );
