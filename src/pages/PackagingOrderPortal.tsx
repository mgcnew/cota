...
export default function PackagingOrderPortal() {
  const { id } = useParams();
    ...
    async function loadOrder() {
      ...
      try {
        const { data: rawData, error: orderErr } = await supabase
          .rpc("get_public_packaging_order_data", { p_order_id: id });
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      const { data: successData, error } = await supabase
        .rpc("public_confirm_packaging_order", { p_order_id: id });
    ...
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col items-center justify-center p-4 font-sans">
        <div className="w-full max-w-md animate-in slide-in-from-bottom-6 fade-in duration-700">
          ...
          <Card className="border-border dark:border-white/5 bg-white dark:bg-zinc-900 shadow-xl rounded-[2rem] overflow-hidden">
            <CardContent className="p-0">
               <div className="p-8 border-b border-border dark:border-white/5 space-y-6 text-left">
                  <div>
                    <span className="text-[9px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest block mb-1">Empresa</span>
                    <p className="text-sm font-black text-zinc-900 dark:text-zinc-50">NOVO BOI JOÃƒO DIAS MERCADÃƒO LTDA</p>
                    <p className="text-[10px] font-bold text-zinc-500 mt-0.5">CNPJ: 63.195.471/0001-12</p>
                  </div>
                  
                  <div className="p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-2xl border border-border dark:border-white/5">
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 flex flex-col transition-colors font-sans">
      ...
      <header className="sticky top-0 z-50 w-full bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl border-b border-border dark:border-white/5 px-4 py-3">
      ...
      <main className="max-w-2xl mx-auto w-full px-4 py-8 pb-32">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
           <div className="bg-white dark:bg-zinc-900 border border-border dark:border-white/5 p-5 rounded-2xl shadow-sm flex items-center gap-4 group hover:border-blue-500/30 transition-colors text-left">
               ...
               return (
                 <div key={i} className="group relative bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 p-4 rounded-xl flex items-center justify-between hover:border-blue-500/40 transition-all shadow-sm">
                   ...
                   <div className="flex-1 min-w-0 pr-4 text-left">
                     ...
                     <div className="flex items-center gap-3">
                        <span className="text-[10px] font-black bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 px-2 min-w-[50px] py-0.5 rounded-md text-center border border-border dark:border-white/5">
