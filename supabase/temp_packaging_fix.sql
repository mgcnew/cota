...
BEGIN
    ...
    LOOP
        INSERT INTO packaging_supplier_items (
            ...
            (v_item->>'unidade'),
            (v_item->>'quantidade_venda')::NUMERIC,
            (v_item->>'quantidade_unidades_estimada')::INTEGER,
            (v_item->>'gramatura')::NUMERIC,
            (v_item->>'dimensoes'),
            CASE 
                WHEN (v_item->>'quantidade_unidades_estimada')::NUMERIC > 0 
                THEN (v_item->>'valor_oferecido')::NUMERIC / (v_item->>'quantidade_unidades_estimada')::NUMERIC 
                ELSE 0 
            END,
            NOW()
        )
